...
/*
 * 256元素查找表占用1024个字节，并且在处理大约30-60个字节的数据之后可以完全缓存它。 
 * 因此，对于短数据，我们使用16元素查找表，该表仅占用64个字节并将其与CPU高速缓存行大小对齐。 
 * 当然，小表在CRC32循环中添加了代码，但是缓存未命中开销大于附加代码的开销。 
 * 例如，16字节数据的ngx_crc32_short（）占用的CPU时钟是ngx_crc32_long（）的一半。
 *
 *
 * */
